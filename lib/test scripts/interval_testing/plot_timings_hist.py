...
def histo(log_path, out_fig, xlabel, ylabel, colour="blue"):
    ...
    return ((high / tot), (popval / 100))

if __name__ == "__main__":
    fracl = [0.0] * 0
    maxl = [0.0] * 0
    logno = [0.0] * 0
    for p in sorted(glob(r"./light sensor/log_hall_*.csv")):
        (frac, maxv) = histo(p, "light_" + p[24:-4] + ".png", r"$Interval, ms$", r"$Occurance$")
        fracl.append(frac)
        maxl.append(maxv)
    ...
    ax1.scatter(logno, maxl, color="red")
    #ax1.plot([logno[0], logno[0] + len(logno) - 1], [0.1, 0.1], color = "red")
    ax1.set_xlabel("$File\ number$", ha='center',
    va='center', fontsize=16)
    ax1.set_ylabel("$Fraction\ of\ intervals\ on\ target$", ha='center',
    va='center', fontsize=16)
    ...
    maxl = [0.0] * 0
    logno = [0.0] * 0
    for p in sorted(glob(r"./hall effect/log_hall_*.csv")):
        (frac, maxv) = histo(p, "hall_" + p[23:-4] + ".png", r"$Interval, ms$", r"$Occurance$")
        fracl.append(frac)
        maxl.append(maxv)
        logno.append(float(p[23:-4]))
    
    print logno[0] + len(logno) - 1
    # Plot (meta) Data
    f = plt.figure(figsize=(10, 10))
    ax1 = f.add_subplot(111)
    ax1.plot(range(0, len(fracl)), fracl)
    ax1.scatter(range(0, len(fracl)), maxl, color="red")
    #ax1.plot([logno[0], logno[0] + len(logno) - 1], [0.1, 0.1], color = "red")
    ax1.set_xlabel("$File\ number$", ha='center',
    va='center', fontsize=16)
    ax1.set_ylabel("$Fraction\ of\ intervals\ on\ target$", ha='center',
    ...
    plt.savefig("meta_plot_hall.png")
