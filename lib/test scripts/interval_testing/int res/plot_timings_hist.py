...
def histo(log_path, out_fig, xlabel, ylabel, colour="blue"):
    ...
    (n, b, p) = ax1.hist(dt, bins, color=colour)
...
if __name__ == "__main__":
    ...
    for p in sorted(glob(r"./log_hall_*.csv")):
        (maxocc, poptime) = histo(p, "light_" + p[9:-4] + ".png", r"$Interval, ms$", r"$Occurance$")
        # fracl.append(frac)
        maxl.append(poptime)
    ...
    ax1.scatter([1, 11, 13, 15, 3, 5, 7, 9], maxl, color="red")
    #ax1.plot([logno[0], logno[0] + len(logno) - 1], [0.1, 0.1], color = "red")
    ax1.set_xlabel("$File\ number$", ha='center',
    va='center', fontsize=16)
    ax1.set_ylabel("$Most\ common\ interval\ time,\ ms$", ha='center',
    va='center', fontsize=16)
