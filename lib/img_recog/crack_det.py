...
# Imports
import math  # s
from PIL import Image  # uses PIL v1.1.7 for python 2.7


def crack_det(path, BGT=245, crack_thresh=15, verbose_level=0, mono=False):
    ...
    for x in range(1, image.size[0] - 1):
        for y in range(1, image.size[1] - 1):
            ...
            if (mono):
                ...
                # calculate, normalise, and invert the length
    ...
    if (verbose_level > 1):
        ...
        if (mono):
            image.save("mono.bmp")
    crack_amnt = ((crack_px * 1000) / total_px)
    if (crack_amnt >= crack_thresh):  # If more than threshhold amount of black (edges), assume cracks found
        if (verbose_level > 0):
            print(("Detection complete (" + str(crack_amnt) + "%, cracks found)"))
        return (True, crack_amnt)
    else:
        if (verbose_level > 0):
            print(("Detection complete (" + str(crack_amnt) + "%)"))
        return (False, crack_amnt)
