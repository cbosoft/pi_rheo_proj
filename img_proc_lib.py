...
class Image_Processor(object):
	...
	def set_img(self, path):
		...
		#self.img_edge = self.edge_det()
		#self.pix_edge = self.img_edge.load()
		self.crack_found = False
	
	# apply edge detection algorithm
	def edge_det(self, BGT=245, crack_thresh=5):
                self.verb_prnt("Edge detection begninning...")
		image = self.img													# set up working variables
		...
		if (image == self.img):							# if the img was the class local, set the local outputs
		...
		self.verb_prnt("Crack amnt:" + str(((crack_px * 100) / total_px)))
		if (self.verbose):
                        self.img_edge.save("edge.bmp")
		return image_edge
	
	# runs edge detection and gets whether a crack was found
	def crack_det(self, image_path, BGT=245, crack_thresh=5):
                self.verb_prnt("Image loaded!")
		self.set_img(image_path)
		self.edge_det(BGT, crack_thresh)
		return self.crack_found

        def verb_prnt(self, mesg):
                if (self.verbose):
                        print mesg
