...
\begin{document}
	...
	\end{itemize}
	\newpage
	\section{Week Beginning 13 March 2017 (Week 10)}
	\begin{itemize}
		\mon
		...
		\subitem Moving average (MAV); exponential decay weighted MAV (gaussian); 
		\subitem Butterworth filter; Wiener probability filter; spline smoothing
		\item Wrote more of the report.
		\tue
		\item Researched signal processing theory and methods.
		\item Started writing up signal processing theory.
		\wed
		\item Noticed that the dynamo is no longer reading values as nicely as before - perhaps the lack of a protection diode has fried the OpAmp? Will add diode, replace OpAmp
		\item Belay last, seems to be working alright now. Will have to keep an eye on that.
		\item Took preliminary torque readings. Seems to be okay, a little noise though.
		\thu 
		\item Updated report
		\fri
		\item Attempted to read torque, unreasonable data recovered. Will try again monday, with filtering on.
		\item Updated report.
	\end{itemize}
	\newpage
	\section{Week Beginning 20 March 2017 (Week 11)}
	\begin{itemize}
		\mon
		\item Torque output curve experiment not yielding sensible results, looked into different ways of obtaining torque information
		\item found a relation between Torque, stall torque, no-load speed, and speed. Just need to find the stall torque.
		\item Discussed structure of report with Dr. lue.
		\tue 
		\item Used balance to gauge stall torque at different current supplies
		\item Updated report.
		\wed
		\item Updated report.
		\thu 
		\item Updated report.
		\fri
		\item Recalibrated dynamo sensor and HES sensor.
		\item Tested air viscosity (zero).
		\item Updated report.
	\end{itemize}
	\newpage
	...
	\begin{itemize}
		...
		\wed 
		\item Updated correlation calculation
		\thu 
		\item Presented work so far. Got advice from Dr. Lue, Vincenzo et al for how to change/improve the presentation/results
		\fri
		\item Logged data for rheometer with various glycerol solutions (98.87v\% to 88.87v\%)
