...
\def\thewords{8151} % last proper build on Wed 15 March 2017 at 15:41:23
\def\br{\newline \newline \noindent}
\def\hi{\huge{!!!}}
\def\nohi{!!!! \normalsize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%% TITLE PAGE
	...
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% MAIN BODY PREAMBLE
	% Summary Page
	\chapter*{Summary} % last edited 14/3/2017
...
%=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++
	...
	\label{expdia}
	\end{figure} \newline \noindent
	%\subsection*{Couette Cell} % last edited 14/3/2017
	% TODO :: UNCERTAINTIES IN MEASUREMENTS, MEASUREMENTS
	The outer cylinder is a hollow glass cylinder, closed at one end. It has an outer diameter of [40]mm, a height of [\cbh], and a thickness of approx. 2mm. The cylinder has been glued to a metal plate which holds it in place. \br
	The inner cylinder is a perspex cylinder built onto the motor's shaft. The cylinder is 30.13mm in diameter, and []mm in height. There is a thinner section of the cylinder at the top of the main cylinder, which is used as a pulley for the belt to hold on to (see Figure \ref{expdia}). This part of the cylinder has a diameter of []mm and is []mm in height.\br
	...
	%\subsection*{Electronics} % last edited 14/3/2017
	...
	The filter parameters were altered until the filterred response for a series of test data closely matched the expected result - indicating that the filter was working optimally. These parameters are set as default in the \texttt{filter} function, but can be changed if desired. \cbh % A=?!!?!?!? B=!?!??!
...
%       \item \texttt{max\_speed} - Unused
	...
	\end{itemize}
	The motor class, once set up, can be used to set the value on the digital potentiometer (thus setting its own supply voltage), can read in its speed (using the ADC), and it can calculate its current draw (using the ADC). \br
...
%=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++
	...
	%Maybe also include a glossary of terms?
	\begin{center}
		\begin{tabular}{|c|l|}
			\hline
...
\begin{figure}[!htb]
	{\centering
	\fbox{\includegraphics[scale=0.35]{figures/fig_filter_demons.png}}
