...
\def\thewords{6535} % last proper build on Fri 24 March 2017 at 16:54:29
\def\br{\newline \newline \noindent}
\def\hi{\huge{!!!}}
\def\nohi{!!!! \normalsize}
...
%=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++
	...
	Raspberry Pis have been widely adopted by the electronic hobbyist groups, used in a wide variety of projects; ranging from simple robotics to home security and automation systems. The latter shows how powerful the Raspberry Pi can be, and how it can be of use in the laboratory. The Raspberry Pi is, via the GPIO ports, high modular in terms of the available hardware. Any amount of sensors can be attached allowing the Raspberry Pi to have access to any amount of information, depending on the needs of the application. \br
	In this application, the hardware connected to the Raspberry Pi was relatively simply. A programmable motor driver circuit, and two sensors (one for rotational speed, and another for motor current supply).\br
...
%=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++
	...
	\begin{figure}[!htb]
		...
		\label{pidia}
	\end{figure} \newline% \noindent
	The first Raspberry Pi (Model A) had a single core 700MHz processor and 256MB of RAM \cite{pi1info}, while the current Raspberry Pi 3 Model B has a quad core 1.2GHz and 1GB of RAM (also including built in WiFi and Bluetooth) \cite{pi3info}. \br
	Despite being initially designed for educational purposes, the Raspberry Pi has found success in other areas such as with hobbyists \cite{pihobbynotedu} and in industry \cite{pimorethanedu}. As a tool in research, the Raspberry Pi has been used as a miniature server for researching the traffic patterns in data centres (the Pis were emulating the behaviours of full scale servers) \cite{piglasgowdc}.\br
...
%=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++
	...
	Software was written to enable the Raspberry Pi to communicate with the hardware. The Python language was chosen as it is a very popular, easy to use, and easy to read language. Its popularity means that there is an abundance of software packages available for inclusion.
		\begin{figure}[!htb]
			\centering
	...
	The average strain rate imposed upon the fluid in the gap between the cylinders can be calculated using Equation \ref{eqnavsr}. The strain rate is the derivative of the velocity of the fluid in the gap. If the velocity varies linearly through the gap (as in ideal Couette flow), then the average derivative will simply be the velocity ($ms^{-1}$) divided by the gap size ($m$).
	\begin{equation}
