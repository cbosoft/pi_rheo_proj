...
\def\thewords{6418} % last proper build on Fri 21 April 2017 at 17:53:19
\def\br{\newline \newline \noindent}
\def\hi{\huge{!!!}}
\def\nohi{!!!! \normalsize}
...
%=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++
	...
	% UNITS IN EMPIRICAL EQUATIONS (DONE)
	% DIMENSIONS SHOWN ON CELL DIAGRAM (IN PROGRESS)
...
%	The design requires that some calibrations are made prior to use. How the motor's speed varies with the voltage output of the dynamo is required before the system can give a speed reading, and the correlation between the current supply and the HES voltage output is required. The motor characteristic curves are also required for the viscosity calculation. This data is found by experimentation as follows.
	...
	\noindent
	\newline
	The reference fluids were then ran in the rheometer. The torque required to shear the fluid at a shear rate was compared to the increase in motor current draw ($\Delta current$). The results were plotted on Figure \ref{figtcal} and a linear equation was fit, giving an equation for motor torque output with respect to the current draw. This was used to calculate the viscosity of other reference fluids. The results were compared (see Figure \ref{figflucalres}) to gauge how well the correlation works, and whether it is valid or not.
	\begin{figure}[!htb]
		\centering
		...
		\begin{subfigure}{0.9\textwidth}
			\footnotesize Motor torque plotted against supply current (blue lines). A first order fit equation is included (green line).
		\end{subfigure}
	\end{figure}
	\begin{figure}[!htb]
		\centering
		...
		\label{figflucalres}
		\begin{subfigure}{0.9\textwidth}
			\footnotesize Viscosity of reference fluids plotted against time. The solid lines represent the expected results for the viscosity (average of the DHR-2's results for the solution) and the symbols represent the calculated viscosities using the rheometer.
		\end{subfigure}
	\end{figure}
	...
	\begin{figure}[!htb]
		\centering
		\includegraphics[scale=0.45]{figures/fig_torque_cal.png}
		\caption{Torque Calibration Results}
		\label{figmottorqcal}
		\begin{subfigure}{0.9\textwidth}
...
\fi
	...
	\noindent
	The theoretically estimated data and the actual viscosity readings differ slightly. This is likely due to water contamination of the glycerol used to produce the solutions. Glycerol is very hydrophillic and will draw moisture out of the air. The viscosity of a glycerol water solution is highly sensitive to the presence of water (increasing water content from 0vol\% to 100vol\% results in the viscosity dropping by 2.3Pa.s) therefore even a tiny amount of water in the glycerol source fluid is going to alter the viscosity hugely. It would be better to use higher viscosity solutions for a better range of values, but the important thing is that the reference solutions are of accurately known viscosity. 
	\br
	The viscosity of the solutions was checked using a DHR-2 cone-and-plate rheometer. A 40mm parallel plate was used. The rheometer was used to sweep up strain rate from 5$s^-1$ to 250$s^-1$, roughly the same range of strain rates from the Raspberry Pi rheometer. A 400$\mu m$ gap was used. It was calculated that 502$\mu l$ of each solution would be required to fill the gap accurately. To account for any small loss of fluid to the pipette, 505$\mu l$ of each solution was pipetted (using a Starlab ErgoOne 20$\mu l$ to 200$\mu l$ single channel pipette) onto the plate. The 40mm plate head was lowered into the test position and the rheometer was run for its set sweep. This was repeated three times per solution tested.
	\br
...
%% APPENDIX %%
	...
	[Unfinished section]
	% TALK ABOUT UNRELIABLE SENSOR - SOMETIMES HES DROPPED OUT AND REPORTED NEGATIVE CURRENT
