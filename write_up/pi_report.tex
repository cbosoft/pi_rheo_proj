...
\def\thewords{6767} % last proper build on Sun 26 March 2017 at 00:24:14
\def\br{\newline \newline \noindent}
\def\hi{\huge{!!!}}
\def\nohi{!!!! \normalsize}
...
%=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++
	...
	%Jamming can be observed as a sudden increase in the apparent viscosity, accompanied by a decrease in rotational speed and increase in current drawn by the motor.\br
...
%		\item \texttt{cvf} = "current-voltage-function", tuple representing the two coefficients in a linear (1D) fit equation for the relationship between the motor's current draw and the voltage read from the HES current sensor via the ADC. \texttt{(3.58, -7.784)} by default.
	...
	\begin{equation}
		T = \left(0.169 \times I_{ms} + 0.038\right) \left(1 - \frac{\omega}{0.01138 pv + 0.3844} \right)
		\label{eqnmottsvsp}
	\end{equation}
...
%	The design requires that some calibrations are made prior to use. How the motor's speed varies with the voltage output of the dynamo is required before the system can give a speed reading, and the correlation between the current supply and the HES voltage output is required. The motor characteristic curves are also required for the viscosity calculation. This data is found by experimentation as follows.
	...
	\begin{figure}[!htb]
		...
		\begin{subfigure}[t]{0.45\textwidth}
			\centering
			\includegraphics[scale=0.35]{figures/fig_speed_v_rvolt.png}
	...
	Figure \ref{figcurevalres} shows how the calibrated current readings (after filtering) compare with actual values obtained using a multimeter. The values are somewhat in agreement, although the data is somewhat noisy, despite the applied filtering.
...
\fi
    ...
    \noindent
	As an initial test, the rheometer was run with no fluid in the cylinder: effectively trying to measure the viscosity of the air. With no load on the inner cylinder (zero torque output), the motor is rotating at its no-load speed. Therefore the shear stress exerted on the air is very small, and the shear rate is very large - resulting in a viscosity reading of zero (or very near). The rheometer ran at different shear rates, steadily increasing over the course of the run. The resulting viscosity data was plotted against time and against shear stress (see Figures \ref{figviscair}). 
	\newline
	...
	\begin{figure}[!htb]
		...
		\begin{subfigure}[t]{0.45\textwidth}
			\centering
			\includegraphics[scale=0.3]{figures/fig_visco_v_t.png}
			\caption{Viscosity vs. Time}
			\label{figviscovt}
			\footnotesize
			Results from an empty run of the rheometer plotted against time (blue circles). Expected result (green dashed line) was zero viscosity for all time.
		\end{subfigure}
		\begin{subfigure}[t]{0.45\textwidth}
			\centering
			\includegraphics[scale=0.3]{figures/fig_visco_v_tau.png}
			\caption{Viscosity vs. Shear Stress}
			\label{figviscovtau}
			\footnotesize
			Results from empty run of the rheometer plotted against shear stress (blue circles). Expected result (green dashed line) for all stresses.
		\end{subfigure}
		\caption{Viscosity Reading Test \label{figviscair}}
	\end{figure}

	\noindent
	In Figure \ref{figviscovt} the viscosity starts very high (approx. 2.5 $Pa.s$) before rapidly falling to around zero. The motor supply voltage is slowly increasing throughout the run; an innacuracy in the speed reading is unlikely to be the source of this error. A more likely source would be in the dynamo calibration (the current supply used to calculate the stall torque could be wrong) or in the stall torque calibration. The latter is a more likely source of the error due to the limited amount of data in that calibration. This is somewhat confirmed in Figure \ref{figviscovtau} - the viscosity appears to be proportional to the shear stress.
	\br
	[Unfinished section]
%=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++=====------++++++
